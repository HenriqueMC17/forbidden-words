...
### 10. Compatibilidade e Blindagem de Upgrades (Zero-Downtime Fix)
*   **Problema**: Se um cliente estiver jogando e a aba de seu navegador mantiver o cache do frontend da versão antiga, a chamada para `rooms:createRoom` ou `rooms:joinRoom` falharia por ausência dos campos `avatar` e `color` no validador de argumentos do Convex.
*   **Solução**: Ajustado o validador de argumentos do backend no Convex (`rooms.ts`) para tornar os campos `avatar` e `color` opcionais (`v.optional(...)`) nas mutações de entrada. Caso o frontend envie uma versão antiga e esses dados estejam ausentes, o backend automaticamente atribui valores padrão (`🦊` e `hsl(262, 83%, 68%)`). Isso previne erros de validação durante o período de atualização do app.

### 11. Correção Crítica de Ordem de Hooks (React Rules of Hooks Fix)
*   **Problema**: Ao criar a sala, a página renderizava em branco devido a uma quebra nas regras de Hooks do React (`Rules of Hooks`). O hook `useCallback` do `handleVoiceTranscript` estava declarado após a verificação condicional de carregamento da query (`if (!room) return;`). Quando a sala começava a carregar, o React executava menos hooks do que no segundo render (onde a sala já existia), gerando um crash crítico em runtime.
*   **Solução**: Movida a declaração do hook `useCallback` de `handleVoiceTranscript` para o topo de `GameRoom.tsx`, acima do bloco de retorno condicional `if (!room)`. Foi adicionada uma blindagem (`if (!room) return;`) interna ao callback para segurança de tipo. O funcionamento e a renderização foram homologados com testes automatizados via Puppeteer headless browser, que confirmou ausência de qualquer erro de console.
